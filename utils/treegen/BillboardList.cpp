...
void CBillboardList::SaveBlb( const char* filename )
{
	...
	for(int i = 0; i < m_sprites.numElem(); i++)
	{
		...
		kvb->AddValue(EqString::Format("%g", m_sprites[i].position.x).ToCString());
		kvb->AddValue(EqString::Format("%g", m_sprites[i].position.y).ToCString());
		kvb->AddValue(EqString::Format("%g", m_sprites[i].position.z).ToCString());
		kvb->AddValue(EqString::Format("%g", m_sprites[i].scale).ToCString());
	}
