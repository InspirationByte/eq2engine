...
void UTIL_ConvertDSMMaterialPath(EqString& path)
{
	...
	char* pszPath = (char*)path.ToCString();
	if (sub)
	{
...
void CMainWindow::ProcessAllMenuCommands(wxCommandEvent& event)
{
	...
	{	
		wxFileDialog* file = new wxFileDialog(NULL, "Open material/atlas file", 
													wxString::Format("%s/materials", g_fileSystem->GetCurrentGameDirectory()), 
													"*.atlas", 
		...
		if(file && file->ShowModal() == wxID_OK)
		{
			...
			EqString fname(file->GetPath().c_str().AsCharBuf());

			fname = fname.Path_Strip_Ext();

			UTIL_ConvertDSMMaterialPath(fname);

			g_treeAtlasGroup.Init(fname.ToCString(), false);
	...
	else if(event.GetId() == Event_File_SaveBillboard)
	{
		...
		wxFileDialog* file = new wxFileDialog(NULL, "Save billboard file", 
													wxString::Format("%s/scripts", g_fileSystem->GetCurrentGameDirectory()), 
													"*.blb", 
		...
		if(file && file->ShowModal() == wxID_OK)
		{
			g_billboardList.SaveBlb(file->GetPath());
		}
	...
	else if(event.GetId() == Event_Edit_GenerateFromObjPoints)
	{
		...
		wxFileDialog* file = new wxFileDialog(NULL, "Open OBJ file", 
			wxString::Format("./", g_fileSystem->GetCurrentGameDirectory()),
			wxEmptyString,
			"Model files (*.obj, *.esm)|*.obj;*.esm;",
			wxFD_FILE_MUST_EXIST | wxFD_OPEN);
		...
		if(file && file->ShowModal() == wxID_OK)
		{
			...
			LoadObjAsPointList(file->GetPath(), points);
	...
	if(event.ShiftDown())
	{
		...
		if(event.ButtonIsDown(wxMOUSE_BTN_MIDDLE))
		{
			if(event.Dragging())
			{
				...
				cam_pos -= right*move_delta_x * camera_move_factor * g_fCamSpeed * g_frametime * 0.01f;
				cam_pos += up*move_delta_y * camera_move_factor * g_fCamSpeed * g_frametime * 0.01f;
