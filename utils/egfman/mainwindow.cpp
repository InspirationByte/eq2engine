...
public:
	...
	void InitializeControls()
	{
		...
		GetMenuById(fileMenu)->add(GetLocalizer()->GetTokenString("TOKEN_OPENESC", "Compile .ESC script"), ACTION_FILE_LOADSCRIPT);
	...
	void redraw()
	{
		...
		if(materials->BeginFrame())
		{
			...
			*/
			...
			for(int i = 0; i < pLoadedModel->GetHWData()->pStudioHdr->numbones; i++)
			{
				bonematrices[i] = pLoadedModel->GetHWData()->joints[i].absTrans;
			}
...
void OpenScript()
{
	...
	char szFilter[80] = "Script for EGFCA (*.ESC)\0*.esc\0All Files (*.*)\0*.*\0\0";
	szFile[0] = '\0';
	szFileTitle[0] = '\0';
	...
	*/
	...
	if(script.LoadFromFile(szFile))
	{
		...
		kvsection_t* mainsection = script.GetRootSection()->FindSection("CompileParams");
		if(mainsection)
		{
			...
			if(pPair)
			{
				g_model_aftercomp_name = pPair->value;
				system(varargs("bin32\\egfca.exe +filename %s", szFile));
			}
			else
			{
