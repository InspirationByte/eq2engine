...
LIB_PATHS=$LIB_PATHS$(pwd)/eq2engine/src_dependency/ffmpeg/lib
...
do
    cp -Lf $(ldd build/bin/${PLATFORM}/${CONFIGURATION}/DrvSyn${EXE_SUFFIX} | awk '/ => / { print $3 }' | grep ${lib}) $DESTINATION_PATH
done
