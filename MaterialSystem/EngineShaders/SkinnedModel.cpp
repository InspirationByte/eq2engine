...
class CSkinnedModel : public CBaseShader
{
public:
	CSkinnedModel()
	{
	...
	void InitTextures()
	{
		...
		// set texture setup
		SetParameterFunctor(SHADERPARAM_BASETEXTURE, &CSkinnedModel::SetupBaseTexture);

		if(m_pBumpTexture)
			SetParameterFunctor(SHADERPARAM_BUMPMAP, &CSkinnedModel::SetupBumpmap);

		if(m_pSpecIllum)
			SetParameterFunctor(SHADERPARAM_SPECULARILLUM, &CSkinnedModel::SetupSpecular);

		SetParameterFunctor(SHADERPARAM_COLOR, &CSkinnedModel::SetColorModulation);
	}
...
DEFINE_SHADER(BaseSkinned, CSkinnedModel)
