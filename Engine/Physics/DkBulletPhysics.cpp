...
#if !defined(EQLC) && !defined(NO_GAME)
...
void DkPhysics::Simulate(float dt, int substeps)
{
	...
	if(m_dynamicsWorld)
	{
		...
		if(ph_debug_serializeworld.GetBool())
		{
			...
			GetFileSystem()->Write(serializer->getBufferPointer(),serializer->getCurrentBufferSize(),1,file);
...
#ifndef NO_GAME
		float fTimeMultipler = sys_timescale.GetFloat()*gpGlobals->timescale;
#else
...
IPhysicsObject* DkPhysics::CreateObject( physmodeldata_t* data, int nObject )
{
	CScopedMutex m(m_Mutex);
