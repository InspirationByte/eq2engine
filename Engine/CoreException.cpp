...
EXPOSE_SINGLE_INTERFACE_NOBASE(m_hCrashPrt,HMODULE)
...
void LoadCrashRpt()
{
	...
	m_hCrashPrt = reinterpret_cast<HMODULE *>(LoadLibrary("CrashRpt.dll"));//FS_LoadModule("CrashRpt.dll");//LoadLibrary( "CrashRpt.dll" );
	if( !m_hCrashPrt )
	{
		//MsgError("Could not load CrashRpt.dll!\n");
		return;
	}

	// fill in the function pointers
	CRASHPRT_INSTALL_FUNC CrashRpt_Install = (CRASHPRT_INSTALL_FUNC)GetProcAddress((HMODULE)m_hCrashPrt, "Install");
	CRASHRPT_ADDFILE_FUNC CrashRpt_AddFile = (CRASHRPT_ADDFILE_FUNC)GetProcAddress((HMODULE)m_hCrashPrt, "AddFile");

	if (!CrashRpt_Install || !CrashRpt_AddFile)
	{
		MsgError("Could not load functions from CrashRpt.dll!\n");
		return;
	}

	LPVOID lpvState = CrashRpt_Install(NULL, NULL, NULL);
	CrashRpt_AddFile(lpvState, "engine.log", "Engine log file");
}
...
void UnloadCrashRpt()
{
	if (m_hCrashPrt)
	{
		FreeLibrary((HMODULE)m_hCrashPrt);
		m_hCrashPrt = NULL;
	}
}
...
void UnInstallExceptionHandler()
{
	...
	{
		MsgError("Removing exception handler...");
