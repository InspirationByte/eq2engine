...
template <typename T, int CHUNK_ITEMS>
struct MemoryPool<T, CHUNK_ITEMS>::Buffer
{
...
template <typename T, int CHUNK_ITEMS>
MemoryPool<T, CHUNK_ITEMS>::~MemoryPool()
{
	Buffer* buffer = m_firstBuffer;
	while (buffer)
	{
...
template <typename T, int CHUNK_ITEMS>
void MemoryPool<T, CHUNK_ITEMS>::clear()
{
	Buffer* buffer = m_firstBuffer;
	while (buffer)
	{
...
template <typename T, int CHUNK_ITEMS>
T* MemoryPool<T, CHUNK_ITEMS>::allocate()
{
	if (m_firstFreeBlock)
	{
...
template <typename T, int CHUNK_ITEMS>
void MemoryPool<T, CHUNK_ITEMS>::deallocate(T* pointer)
{
	if (!pointer)
		return;
