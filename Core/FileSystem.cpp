...
int CFileSystem::Seek( DKFILE *fp, long pos, int seekType )
{
	if(!fp)
		return -1;

	return fp->Seek(pos, (VirtStreamSeek_e)seekType);
}

long CFileSystem::Tell( DKFILE *fp )
{
	if(!fp)
		return 0;
...
size_t CFileSystem::Read( void *dest, size_t count, size_t size, DKFILE *fp )
{
	if(!fp)
		return 0;

	return fp->Read(dest, count, size);
}

size_t CFileSystem::Write( const void *src, size_t count, size_t size, DKFILE *fp )
{
	if(!fp)
		return 0;

	return fp->Write(src, count, size);
}

size_t CFileSystem::Printf( DKFILE *fp, const char *fmt, ... )
{
	if(!fp)
		return 0;
	...
	va_start (argptr, fmt);
	int wcount = vsprintf(string, fmt, argptr);
	va_end (argptr);

	return fp->Write(string, 1, wcount);
}

int CFileSystem::Error( DKFILE *fp )
{
	if(!fp)
		return 0;
...
int CFileSystem::Flush( DKFILE *fp )
{
	if(!fp)
		return 0;
...
char *CFileSystem::Gets( char *dest, int destSize, DKFILE *fp )
{
	if(!fp)
		return 0;
	...
	if(fp->GetType() == VS_TYPE_FILE)
	{
		...
		return pFile->Gets(dest, destSize);
	}
	else if(fp->GetType() == VS_TYPE_FILE_PACKAGE)
	{
		CVirtualDPKFile* pFile = (CVirtualDPKFile*)fp;
...
#ifdef _WIN32
int CFileSystem::Stat( const char *path, struct _stat *buf, int searchFlags/* = -1*/  )
{
    int rt = _stat(path, buf);
    return rt;
}
#else
int CFileSystem::Stat( const char *path, struct stat *buf, int searchFlags/* = -1*/  )
{
...
#ifdef _WIN32
HANDLE CFileSystem::FindFirstFile(char *findname, WIN32_FIND_DATA *dat, int searchFlags/* = -1*/ )
{
    return ::FindFirstFile(findname, dat);
}

bool CFileSystem::FindNextFile(HANDLE handle, WIN32_FIND_DATA *dat )
{
    return (::FindNextFile(handle, dat) != 0);
}

bool CFileSystem::FindClose(HANDLE handle)
{
