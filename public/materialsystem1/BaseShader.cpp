...
MatTextureProxy CBaseShader::LoadTextureByVar(const char* paramName, bool errorTextureIfNoVar, int texFlags)
{
	...
	if(mv.IsValid())
	{
		if (mv.Get().Length())
		{
			...
			ITexturePtr texture = g_texLoader->LoadTextureFromFileSync(mv.Get(), SamplerStateParams(m_texFilter, m_texAddressMode), loadFlags, EqString::Format("Material %s var '%s'", m_material->GetName(), paramName));
			if (texture)
			{
