...
bool EqLua_GetTableSafe(lua_State* state, OOLUA::Table& table, int index)
{
	LuaStackGuard g(state);

	// получить таблицу как результат выполнения функции
	if (lua_isnil(state, -1))
	{
		MsgError("EqLua_GetTableSafe error: table is NIL\n");
		return false;
	}
	else if(!lua_istable(state, -1))
	{
		MsgError("EqLua_GetTableSafe error: not a table\n");
		return false;
	}
...
static int eqluadump_writer( lua_State *L, const void *p, size_t size, void *u )
{
	IVirtualStream* pStream = (IVirtualStream*)u;

	int count = pStream->Write(p, size, 1);

    return !(count > 0) && (size != 0);
}

/*
int EqLua_Dump(lua_State *L, lua_Writer writer, void *data, bool strip_source)
{
	int status;
	TValue *o;
	lua_lock(L);
	...
	if ( isLfunction(o) )
		status = luaU_dump(L, getproto(o), writer, data, strip_source);
	else
		status = 1;
...
{
	OOLUA::Script temp_state;

	//LuaBinding_Initialize(temp_state);

	LuaStackGuard g(state);

    if( luaL_loadstring( temp_state, pszBuffer ) != 0 )
	{
		OOLUA::INTERNAL::set_error_from_top_of_stack_and_pop_the_error( temp_state );

		MsgError("EqLua_CompileBuffer error:\n%s\n", OOLUA::get_last_error(temp_state).c_str());
		return false;
    }

	// release stack after compile
	g.Release();

	Proto* f = toproto( temp_state.state(), -1 );

    lua_lock( temp_state );

	//luaU_dump( temp_state, f, ildump_writer, pStream, 1);

	// использовать luaU_dump вместо lua_dump, потому-что она не сбрасывает исходный код
	EqLua_Dump( temp_state, eqluadump_writer, pStream, true);

    lua_unlock( temp_state );
...
{
	char* data = GetFileSystem()->GetFileBuffer(filename, NULL, -1);

	if(!data)
	{
		MsgError("EqLua_CompileFile: can't open file '%s'\n", filename);
		return false;
	}

	bool status = EqLua_CompileBuffer(state, data, pStream);
