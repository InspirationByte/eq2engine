...
void SoundScriptDesc::ParseDesc(SoundScriptDesc& scriptDesc, const KVSection* scriptSection, const KVSection* defaultsSec)
{
	...
	for (int i = 0; i < scriptSection->KeyCount(); ++i)
	{
		...
		auto parseOutputParameter = [&](ESoundParamType paramType) {
			if (stricmp(valKey.name, s_soundParamNames[paramType]))
				return;
		...
		parseOutputParameter(SOUND_PARAM_SAMPLE_VOLUME);
		parseOutputParameter(SOUND_PARAM_SAMPLE_PITCH);
	...
	for (int paramId = 0; paramId < SOUND_PARAM_COUNT; ++paramId)
	{
		...
		char nodeNameWithPrefix[32]{ 0 };
		snprintf(nodeNameWithPrefix, sizeof(nodeNameWithPrefix), "out_%s", s_soundParamNames[paramId]);
