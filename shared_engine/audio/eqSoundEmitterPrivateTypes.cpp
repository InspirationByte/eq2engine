...
static void InitSoundNode(SoundNodeDesc& nodeDesc, const ESoundNodeType type, const int paramType = -1, int inputCount = 1, int outputCount = 1)
{
	nodeDesc.type = type;
...
void SoundScriptDesc::ParseDesc(SoundScriptDesc& scriptDesc, const KVSection* scriptSection, const KVSection* defaultsSec)
{
	...
	auto makeSampleParameter = [&](const KVSection& valKey, ESoundParamType paramType) {
		...
		else
		{
			...
			if (!nodeDesc.type != SOUND_NODE_FUNC)
			{
				...
				nodeDesc.func.type = SOUND_FUNC_COPY;
				nodeDesc.func.inputCount = inputCount;
				nodeDesc.func.outputCount = inputCount;
			}
		}
	};

	// parse inputs, mixers, outputs
	for (KVKeyIterator it(scriptSection); !it.atEnd(); ++it)
	{
		const KVSection& valKey = *(*it);
		if (valKey.IsSection())
			continue;
		...
		if (!stricmp(valKey.name, s_soundParamNames[SOUND_PARAM_SAMPLE_VOLUME]))
			makeSampleParameter(valKey, SOUND_PARAM_SAMPLE_VOLUME);

		if (!stricmp(valKey.name, s_soundParamNames[SOUND_PARAM_SAMPLE_PITCH]))
	...
	for (int paramType = 0; paramType < SOUND_PARAM_COUNT; ++paramType)
	{
		...
		if (nodeId != SOUND_VAR_INVALID)
		{
			...
			InitSoundNode(nodeDesc, SOUND_NODE_FUNC, paramType);
