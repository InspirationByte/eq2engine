...
// Callback that returns null-terminated string allocated with malloc.
typedef char* (*stb_include_load_file_cb)(char* filename, size_t* plen, void* user_data);

// Do include-processing on the string 'str'. To free the return value, pass it to free()
char* stb_include_string(char* str, char* inject, char* path_to_includes, char* filename_for_line_directive, stb_include_load_file_cb load_file_cb, void* user_data, char error[256]);

// Concatenate the strings 'strs' and do include-processing on the result. To free the return value, pass it to free()
char* stb_include_strings(char** strs, int count, char* inject, char* path_to_includes, char* filename_for_line_directive, stb_include_load_file_cb load_file_cb, void* user_data, char error[256]);
...
char* stb_include_file(char* filename, char* inject, char* path_to_includes, stb_include_load_file_cb load_file_cb, void* user_data, char error[256]);
...
char* stb_include_string(char* str, char* inject, char* path_to_includes, char* filename, stb_include_load_file_cb load_file_cb, void* user_data, char error[256])
{
	char temp[4096];
	...
	for (i = 0; i < num; ++i) {
		...
		if (inc_list[i].filename == 0) {
			...
			strcat(temp, "/");
			strcat(temp, inc_list[i].filename);
			inc = stb_include_file(temp, inject, path_to_includes, load_file_cb, user_data, error);
			if (inc == NULL) {
...
char* stb_include_strings(char** strs, int count, char* inject, char* path_to_includes, char* filename, stb_include_load_file_cb load_file_cb, void* user_data, char error[256])
{
	char* text;
	char* result;
	int i;
	...
	result = stb_include_string(text, inject, path_to_includes, filename, load_file_cb, user_data, error);
	free(text);
	return result;
}
...
char* stb_include_file(char* filename, char* inject, char* path_to_includes, stb_include_load_file_cb load_file_cb, void* user_data, char error[256])
{
	size_t len;
	char* result;
	char* text = load_file_cb(filename, &len, user_data);
	if (text == NULL) {
