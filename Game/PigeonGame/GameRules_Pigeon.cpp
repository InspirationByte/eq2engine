...
#include "GameRules_Pigeon.h"
#include "PlayerSpawn.h"

int nGPFlags = 0;
int nGPLastFlags = 0;

DECLARE_CMD( sparta ,"fly through all", CV_CHEAT | CV_CLIENTCONTROLS)
{
...
DECLARE_CMD( kyrlyk, "toggle invicibility mode", CV_CHEAT | CV_CLIENTCONTROLS)
{
	nGPLastFlags = nGPFlags;
...
DECLARE_CMD( imwhitepigeon, "toggles invisible mode", CV_CHEAT | CV_CLIENTCONTROLS)
{
	nGPLastFlags = nGPFlags;
...
{
	g_pGameRules = (IGameRules*)entityfactory->CreateEntityByName("pigeon_gamerules");
	g_pGameRules->Spawn();
...
void CPigeonGameRules::SetLocalPlayer(BaseEntity* pPlayer)
{
	m_pActorList.insert((CBaseActor*)pPlayer, 0);
	GR_SetViewEntity( pPlayer );
}

void CPigeonGameRules::RespawnItems()
{

}

void CPigeonGameRules::UpdateGameRules()
{

}

void CPigeonGameRules::KillActor(int id)
{
...
CBaseActor* CPigeonGameRules::GetPlayerActorById(int id)
{
	if(m_pActorList.numElem() == 0)
		return NULL;
...
CBaseActor* CPigeonGameRules::GetLocalPlayer()
{
	if(m_pActorList.numElem() == 0)
		return NULL;
...
void CPigeonGameRules::AddPlayerStart(CInfoPlayerStart* pStart)
{
	m_pPlayerStarts.append(pStart);
}

void CPigeonGameRules::ProcessDamageInfo( damageInfo_t& damage )
{
	if(damage.m_pTarget)
	{
...
void CPigeonGameRules::InitRelationShips()
{
	/*
...
RelationShipState_e	CPigeonGameRules::GetRelationshipState(EntRelClass_e nClass, EntRelClass_e nRelatedClass)
{
...
void CPigeonGameRules::SetRelationshipState(EntRelClass_e nClass, EntRelClass_e nRelatedClass, RelationShipState_e state)
{
	m_nRelationShipStates[nClass][nRelatedClass] = state;
}

DECLARE_ENTITYFACTORY(pigeon_gamerules,CPigeonGameRules)